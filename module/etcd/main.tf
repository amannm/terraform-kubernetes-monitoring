locals {
  ...
  collect_member() {
      ...
      etcdctl member list | grep http://$${HOSTNAME}.${var.service_name}:${local.peer_port} | cut -d':' -f1 | cut -d'[' -f1 > ${local.data_volume_mount_path}/member_id
      exit 0
  }
  eps() {
      EPS=""
      for i in $$(seq 0 $$((${local.cluster_size} - 1))); do
          EPS="$${EPS}$${EPS:+,}http://${var.service_name}-$${i}.${var.service_name}:${local.client_port}"
      done
      echo $${EPS}
  }
  member_hash() {
      etcdctl member list | grep http://$${HOSTNAME}.${var.service_name}:${local.peer_port} | cut -d':' -f1 | cut -d'[' -f1
  }
  ...
  if [ -e ${local.data_volume_mount_path}/default.etcd ]; then
      ...
      # re-join member
      ETCDCTL_ENDPOINT=$$(eps) etcdctl member update $${member_id} http://$${HOSTNAME}.${var.service_name}:${local.peer_port}
      exec etcd --name $${HOSTNAME} \
          --listen-peer-urls http://$${HOSTNAME}.${var.service_name}:${local.peer_port} \
          --listen-client-urls http://$${HOSTNAME}.${var.service_name}:${local.client_port},http://127.0.0.1:${local.client_port} \
          --advertise-client-urls http://$${HOSTNAME}.${var.service_name}:${local.client_port} \
  ...
  if [ "$${SET_ID}" -ge $${INITIAL_CLUSTER_SIZE} ]; then
      ...
      etcdctl member add $${HOSTNAME} http://$${HOSTNAME}.${var.service_name}:${local.peer_port} | grep "^ETCD_" > ${local.data_volume_mount_path}/new_member_envs
      if [ $$? -ne 0 ]; then
          echo "Exiting"
      ...
      exec etcd --name $${HOSTNAME} \
          ...
          --advertise-client-urls http://$${HOSTNAME}.${var.service_name}:${local.client_port} \
          --data-dir ${local.data_volume_mount_path}/default.etcd \
          --initial-advertise-peer-urls http://$${HOSTNAME}.${var.service_name}:${local.peer_port} \
